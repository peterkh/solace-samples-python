...
from solace.messaging.config.solace_properties.message_properties import (
...
from solace.messaging.config.authentication_strategy import (
...
transport_security = TLS.create().with_certificate_validation(
    ...
    trust_store_file_path=os.environ.get("SOLACE_TRUST_STORE_DIR") or "ca/",
)
...
messaging_service = (
    ...
    .with_authentication_strategy(
        ClientCertificateAuthentication.of(
            key_file=os.environ.get("SOLACE_CLIENT_KEY") or "ca/client.enc.key",
            key_password=os.environ.get("SOLACE_CLIENT_KEY_PASS") or "password",
            certificate_file=os.environ.get("SOLACE_CLIENT_CERT") or "ca/client.pem",
        )
    )
    .build()
)
...
message_body = "Hello from Python Hello World Sample!"
message_builder = (
